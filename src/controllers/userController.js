...
// Cleans up everything that would otherwise dangle when a user (technician
// or admin) is deleted:
//  - Any task the user currently holds that isn't finished yet is returned
//    to the open pool instead of being stuck referencing a nonexistent user.
//  - Completed tasks are left as historical records (their assignedTo ref
//    will populate as null, which is an acceptable trade-off for history).
//  - 1:1 chats always involve exactly one other person, so once this user
//    is gone the conversation can't function — delete the chat and its
//    messages rather than leave a half-populated thread behind.
//  - Notifications addressed to this user are removed.
...
const deleteAccount = asyncHandler(async (req, res) => {
    await cascadeDeleteUserData(req.user._id);
...
const deleteTechnician = asyncHandler(async (req, res) => {
    ...
    await cascadeDeleteUserData(technician._id);
    await User.findByIdAndDelete(technician._id);
